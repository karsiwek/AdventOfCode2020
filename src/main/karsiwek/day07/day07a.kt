...
fun getSumOfanswers(input: String) : Set<Char> {
...
fun parse(input: String) : Map<String, List<String>> {
    ...
    for (line in lines) {
        ...
        if(words[4] == "no") {
            map.put(words[0] + words[1], emptyList());
        } else {
            var list = arrayListOf<String>();

            var i=4;
            while(i < words.size) {
                for(j in 1..words.get(i++).toInt()) {
                    list.add(words[i] + words[i+1]);
                }
                i+=3;
            }
            map.put(words[0] + words[1], list);
        }
    }
...
fun calculatePossibilities(map: Map<String, List<String>>): Map<String, Set<String>> {
    return map.entries.flatMap { elem -> elem.value.map { Pair(it, elem.key) } }
        .groupBy { it.first }
        .map { Pair(it.key, it.value.map { it.second }.toSet()) }
        .toMap()

}

fun getSet(map: Map<String, Set<String>>, colour: String) : Set<String> {
    if(map.containsKey(colour)) {
        return map.get(colour)!!.map { setOf(it) + getSet(map, it) }.foldRight(emptySet(), {a,b -> a+b});
    }
...
fun main(args: Array<String>) {
    System.out.println(getSet(calculatePossibilities(parse(input)), "shinygold").size);
}
